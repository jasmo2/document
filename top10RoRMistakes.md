...
~~ Assigning complex logic inside the controller layer usually breaks the DRY principle by repeating a concern across multiple controllers. Instead, these kind of logic should be down into the model layer where the application core should be. ~~
For instance, calling an  object several times on a single controller gives us the sense on code smell meaning it could be refactor:
